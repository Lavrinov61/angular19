-- ============================================================
-- Wave 2: Materialized View crm_inbox_view
-- Unified inbox: chats + tasks + bookings + orders + approvals
-- Применять: sudo -u postgres psql -d magnus_photo_db -f crm_inbox_mv.sql
-- Refresh: REFRESH MATERIALIZED VIEW CONCURRENTLY crm_inbox_view
-- ============================================================

-- Drop if exists (для переприменения)
...
SELECT
  ...
  COALESCE(
    (SELECT m.content FROM visitor_chat_messages m
  ...
  jsonb_build_object(
    'messageCount', (SELECT COUNT(*) FROM visitor_chat_messages WHERE session_id = s.id)::int,
    'channel',           s.channel,
...
-- ── Indexes ────────────────────────────────────────────────
-- UNIQUE обязателен для REFRESH CONCURRENTLY
...
\echo '✅ crm_inbox_view created with indexes and grants'

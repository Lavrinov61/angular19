-- ============================================================
-- Phase 5 perf: crm_inbox_view — LATERAL JOIN вместо correlated subqueries
-- Убирает N correlated subqueries для preview и messageCount
-- Применять: sudo -u postgres psql -d magnus_photo_db -f phase5_mv_lateral_join.sql
-- ============================================================
...
-- LATERAL JOIN для последнего сообщения (заменяет correlated subquery)
LEFT JOIN LATERAL (
  SELECT content
...
-- LATERAL JOIN для количества сообщений
LEFT JOIN LATERAL (
...
\echo '✅ crm_inbox_view recreated with LATERAL JOIN optimization'

-- Fix: MV crm_inbox_view читала ТОЛЬКО из conversations (v2),
-- но реальные чаты живут в visitor_chat_sessions (v1).
-- Reconciliation удаляла v1 записи из crm_inbox каждые 5 мин.
--
-- Решение: добавить UNION ALL для visitor_chat_sessions (v1)
-- с дедупликацией через legacy_session_id.

DROP MATERIALIZED VIEW IF EXISTS crm_inbox_view;

CREATE MATERIALIZED VIEW crm_inbox_view AS

-- ═══ v1 chats (visitor_chat_sessions) ═══
SELECT
    (s.id)::text AS id,
    'chat'::text AS type,
...
WHERE s.status IN ('open', 'waiting', 'active')
  -- Исключаем сессии, которые уже мигрированы в v2 (conversations)
  AND NOT EXISTS (
      SELECT 1 FROM conversations c WHERE c.legacy_session_id = s.id
  )

UNION ALL

-- ═══ v2 chats (conversations) ═══
SELECT
    (s.id)::text AS id,
    'chat'::text AS type,
    COALESCE(cl.display_name, s.visitor_name) AS client_name,

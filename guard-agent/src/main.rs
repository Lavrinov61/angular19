...
/// Type alias so commands.rs and scanner.rs can keep `use crate::AgentState`.
pub type AgentState = agent::GuardAgentState;

// ── Platform-specific entry points ──

#[cfg(not(windows))]
fn main() -> anyhow::Result<()> {
    svf_agent_core::runner::AgentRunner::new(agent::GuardAgent).run()
}
...
fn main() -> anyhow::Result<()> {
    ...
    if console {
        ...
        svf_agent_core::logging::init("svf_guard_agent", false);
...
fn windows_service_main() -> anyhow::Result<()> {
    ...
    service_dispatcher::start("SvfGuardAgent", ffi_service_main)?;
    Ok(())
}
...
fn service_main(_args: Vec<std::ffi::OsString>) {
    ...
    let status_handle =
        match service_control_handler::register("SvfGuardAgent", event_handler) {
            Ok(h) => h,
            Err(e) => {
    ...
    let runner = svf_agent_core::runner::AgentRunner::new(agent::GuardAgent);

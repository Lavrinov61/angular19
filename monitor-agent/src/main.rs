...
/// Type aliases so commands.rs and other modules can keep `use crate::AgentState`.
pub type AgentState = agent::MonitorAgentState;
pub use agent::MonitorConfig;

// ── Platform-specific entry points ──

#[cfg(not(windows))]
fn main() -> anyhow::Result<()> {
    svf_agent_core::runner::AgentRunner::new(agent::MonitorAgent).run()
}
...
fn main() -> anyhow::Result<()> {
    ...
    if console {
        ...
        svf_agent_core::logging::init("svf_monitor_agent", false);
...
fn windows_service_main() -> anyhow::Result<()> {
    ...
    service_dispatcher::start("SvfMonitorAgent", ffi_service_main)?;
    Ok(())
}
...
fn service_main(_args: Vec<std::ffi::OsString>) {
    ...
    let status_handle =
        match service_control_handler::register("SvfMonitorAgent", event_handler) {
            Ok(h) => h,
            Err(e) => {
    ...
    let runner = svf_agent_core::runner::AgentRunner::new(agent::MonitorAgent);
